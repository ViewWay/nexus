...
Spring Framework support for the RSocket protocol.

This section describes Spring Framework’s support for the RSocket protocol.

## [](#rsocket-overview)Overview
...
### [](#rsocket-java)Java Implementation
...
## [](#rsocket-requester)RSocketRequester
...
### [](#rsocket-requester-client)Client Requester
...
#### [](#rsocket-requester-client-setup)Connection Setup
...
#### [](#rsocket-requester-client-strategies)Strategies
...
#### [](#rsocket-requester-client-advanced)Advanced
...
### [](#rsocket-requester-server)Server Requester
...
| ****2** |
| Perform handling and return completion `Mono`. |
...
### [](#rsocket-requester-requests)Requests
...
```java hljs
Mono location = requester.route("find.radar.EWR"))
...
For `Fire-and-Forget` use the `send()` method that returns `Mono`. Note that the `Mono`
...
## [](#rsocket-annot-responders)Annotated Responders
...
### [](#rsocket-annot-responders-server)Server Responders
...
### [](#rsocket-annot-responders-client)Client Responders
...
### [](#rsocket-annot-messagemapping)@MessageMapping
...
| `@Header`
 ...
 |
| Metadata value registered for extraction as described in [MetadataExtractor](#rsocket-metadata-extractor).
...
| `@Headers Map`
 ...
 |
| All metadata values registered for extraction as described in [MetadataExtractor](#rsocket-metadata-extractor).
...
either `void` or a no-value asynchronous type such as `Mono`.
...
| 1
 ...
 |
| Either an explicit value, or a single-value asynchronous type such as `Mono`.
...
| Many
 ...
 |
| A multi-value asynchronous type such as `Flux`.
...
| For input this means the method does not have an `@Payload` argument.
 ...
 For output this is `void` or a no-value asynchronous type such as `Mono`.
...
### [](#rsocket-annot-rsocketexchange)@RSocketExchange
...
public interface RadarsService {
 ...
 Flux radars(MapRequest request);
}
...
class RadarsController : RadarsService {
 ...
 override fun radars(request: MapRequest): Flow {
 // ...
 }
}
```
...
### [](#rsocket-annot-connectmapping)@ConnectMapping
...
`Mono` as the return value. If handling returns an error for a new
...
## [](#rsocket-metadata-extractor)MetadataExtractor
...
interface RadarService {
 ...
 Flux getRadars(@Payload MapRequest request);
...
### [](#rsocket-interface-method-parameters)Method Parameters
...
### [](#rsocket-interface-return-values)Return Values
